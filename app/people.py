...
class People:
    ...
    @classmethod
    def generate_dataset(cls,n: int = 1,
        ...
        f_fname_df = pd.read_csv(cls.path_dict['f_fname'])
        f_lname_df = pd.read_csv(cls.path_dict["f_lname"])
        m_fname_df = pd.read_csv(cls.path_dict["m_fname"])
        m_lname_df = pd.read_csv(cls.path_dict["m_lname"])

        pop_age = pd.read_json(cls.path_dict['age'])
        ...
        if only_females:
            ...
            result['first name'] = cls.generate_names(
                result, f_fname_df, None, number_of_fnames, unregular_number_of_fnames)

            result['last name'] = cls.generate_names(
        ...
        # drawing a gender
        result['gender'] = pd.Series([cls.generate_gender().value
    ...
    @ staticmethod
    def generate_gender(gender: Tuple[Any] = tuple(gen for gen in gender_enum),
    ...
    @ staticmethod
    def generate_names(df: pd.DataFrame,
        ...
        if not isinstance(second_names_set, pd.DataFrame):
            if (unregular_number_of_names is None):
                return df.apply(lambda _: __class__.concatenate_names(draw_from_df(first_names_set, k=randint(1, number_of_names, sep=name_separator))))
            return df.apply(lambda _: __class__.concatenate_names(draw_from_df(first_names_set, k=number_of_names)[0], sep=name_separator), axis=1)

        # random (1:number of names) number of names
        if unregular_number_of_names and (number_of_names > 1):
            return df.apply(lambda x: __class__.concatenate_names(draw_from_df(first_names_set, k=randint(1, number_of_names))[0], sep=name_separator)
                            if x.gender == 'female'
                            else __class__.concatenate_names(draw_from_df(second_names_set, k=randint(1, number_of_names))[0], sep=name_separator), axis=1)

        # constant number of names
        return df.apply(lambda x: __class__.concatenate_names(draw_from_df(first_names_set, k=number_of_names)[0], sep=name_separator)
                        if x.gender == 'female'
                        else __class__.concatenate_names(draw_from_df(second_names_set, k=number_of_names)[0], sep=name_separator), axis=1)




    def test_voivodship(age: int = None, voivodeship_population_dataset: pd.DataFrame = None, equal_weight: bool = False):
        return cls.__draw_voivodship(age=age, voivodeship_population_dataset=voivodeship_population_dataset, equal_weight=equal_weight)


    def concatenate_names(input:Union[str,List[str], Tuple[str]], sep=" ")->str:
