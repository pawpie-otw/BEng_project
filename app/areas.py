...
class Areas:
    ...
    def generate_dataset(cls,
        ...
        if "voivodeship" in required_cols:
            result["voivodeship"] = cls.complete_voivodeship(
                rows, voivodeship, base_df, required_cols)

        # POSTCODE
        if "postcode" in required_cols:
            result["postcode"] = cls.complete_postcode(
                rows, postcode, result, required_cols)
